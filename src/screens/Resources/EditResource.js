...
const EditResource = ({navigation, route}) => {
  ...
  const clientsOptions = newData.filter(t => t.vendor_id !== null);
  ...
  return (
    <SafeAreaView style={GLOBALSTYLES.mainContainer}>
      ...
      <ScrollView>
        ...
        <View
          ...
          <Picker
            ...
            {experienceOptions.map((item, index) => (
              <Picker.Item
                key={item.exp_date.id}
                label={item.exp_date}
                value={item.exp_date.id}
              />
            ))}
          </Picker>
        </View>
        ...
        <TouchableOpacity style={GLOBALSTYLES.buttonStyle} onPress={putUser}>
